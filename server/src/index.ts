...
// ── API: Import custom word list from CSV ────────────────────────
// CSV format: word,phonetic,pos,meaning,example,exampleCn,difficulty(1-3)

app.post('/api/v1/word-lists/import', (req, res) => {
  const { name, description, csvData } = req.body as {
    name: string;
    description: string;
    csvData: string;
  };
  ...
  if (!name || !csvData) {
    return res.status(400).json({ error: 'name and csvData are required' });
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!line || (i === 0 && line.toLowerCase().includes('word,'))) continue;
    ...
    words.push({
      ...
      example: parts[4] || '',
      exampleCn: parts[5] || '',
      difficulty: (parseInt(parts[6]) as 1 | 2 | 3) || 1,
    });
  }
  ...
  const newList: WordList = {
    id: listId,
    name,
    description: description || `自定义词表 - ${words.length} 词`,
    icon: 'book',
    color: '#10B981',
    words,
  };

  WORD_LISTS.push(newList);

  res.json({
    success: true,
    listId,
    wordCount: words.length,
    list: { id: newList.id, name: newList.name, description: newList.description, icon: newList.icon, color: newList.color },
  });
});

// ── API: Delete custom word list ─────────────────────────────────

app.delete('/api/v1/word-lists/:id', (req, res) => {
  const listId = req.params.id;
  const idx = WORD_LISTS.findIndex(l => l.id === listId);
  if (idx === -1) {
    return res.status(404).json({ error: 'Word list not found' });
  }
  if (!listId.startsWith('custom_')) {
    return res.status(400).json({ error: 'Cannot delete built-in word list' });
  }
  WORD_LISTS.splice(idx, 1);
  listState.delete(listId);
  res.json({ success: true });
});
